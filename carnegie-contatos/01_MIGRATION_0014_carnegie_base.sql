...
alter table contacts
  -- Tier (cadência relacional, separado de phase comercial)
  ...
  -- Cache da última interação (mantido por trigger)
  ...
  -- Último sinal capturado (post, promoção, mudança)
...
-- Trigger para atualizar favor_balance baseado em interaction_tags
...
-- ============================================================
-- Datas especiais expandidas. Tipos:
--   celebrate    → push no dia, sugestão de mensagem festiva
--   acknowledge  → in-app no Pulso (aniv. de conhecimento, "STATE-versary")
--   silence      → silencia gatilhos festivos por N dias (luto, divórcio)
--   check_in     → discreto, sem template (aniv. de luto recorrente)
...
create table if not exists special_dates (
  ...
  -- Recorrente (DD/MM) OU única (date completa). Sempre 1 dos 2.
  ...
  -- Se type=silence, por quantos dias silencia gatilhos festivos
  silence_days    int,

  -- Contexto sensível (não vai para prompts IA)
  private_note    text,
  ...
  -- Pelo menos uma data preenchida
...
-- 5. Tabela: referrals
-- ============================================================
-- Loop de indicação rastreável.
-- "from_contact me indicou for_what (que virou to_contact_id se aplicável)"
...
-- ============================================================
-- Elogios concretos recebidos, com timing deliberado de retribuição
-- (4-6 meses depois — não troca imediata).
...
create table if not exists compliments_received (
  ...
  -- Quando o sistema deve sugerir retribuição (random entre +120 e +180 dias)
...
-- Trigger: ao inserir compliment, calcula remind_to_reciprocate_at
-- random entre +120 e +180 dias
