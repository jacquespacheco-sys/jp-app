...
-- ============================================================
-- Um princípio por mês × user. Banner persistente no Pulso.
-- Métrica: applied_count vai sendo incrementado quando interactions
-- são marcadas com o tag correspondente (ex: 'P7').
...
create table if not exists principle_of_month (
  ...
  -- Meta de aplicações no mês (default 12 = ~3 por semana)
  target_applications int not null default 12,

  -- Reflexão de fim de mês (texto livre)
  reflection      text,
...
-- ============================================================
-- 1 reflexão por semana × user. 3 perguntas curtas.
-- Domingo 19h (cron envia lembrete).
...
create table if not exists weekly_reflections (
  ...
  -- formato ISO week, ex: "2026-W19"

  -- Q1: Quem te marcou esta semana?
  marked_me_contact_id uuid references contacts(id) on delete set null,
  marked_me_why        text,

  -- Q2: Quem você decepcionou?
  let_down_contact_id  uuid references contacts(id) on delete set null,
  let_down_why         text,

  -- Q3: Quem você quer reaproximar? (vira card no Pulso de segunda)
...
-- 3. gratitude_entries
-- ============================================================
-- Diário de gratidão. Sexta 18h push. Não é mandado pra ninguém.
-- 280 chars max (enforcement no schema Zod e no DB).
...
create table if not exists gratitude_entries (
  ...
  -- Marca se a entrada foi compartilhada (mandada pra pessoa) ou só diário
...
-- ============================================================
-- Usada na retrospectiva anual e no Thank You Tour para pré-seleção
